...
// Core Functions
//=============================================================================
function getPluginParameters () {var a = document.currentScript || (function() { var b = document.getElementsByTagName('script'); return b[b.length - 1]; })(); return PluginManager.parameters(a.src.substring((a.src.lastIndexOf('/') + 1), a.src.indexOf('.js')));}
//=============================================================================
//=============================================================================
...
// Parameter Variables
//=============================================================================
CTB.Parameters = getPluginParameters(); var ctb = ctb || {};
...
var _gmamp_ctb = Game_Map.prototype.addMapProjectile; Game_Map.prototype.addMapProjectile = function(object) {if (this._mapProjectiles)_gmamp_ctb.call(this, object);};

// Alias
var _ti_c_ctb = TouchInput.clear;
...
TouchInput.isCancelled = function() {
  ...
  SceneManager._scene._messageWindow = SceneManager._scene._messageWindow || {};

  if (ctb.disableRightClick && !$gamePlayer._mainMenu.inMenu && SceneManager._scene instanceof Scene_Map && !(SceneManager._scene._messageWindow._openness > 0) && !(SceneManager._scene._messageWindow.isAnySubWindowActive())){
    var goOn = true;
    if (ctb.disableRightClick_SwitchID) goOn = ($gameSwitches) ? $gameSwitches.value(ctb.disableRightClick_SwitchID) : false;
    if (!goOn){
...
if (!(SceneManager._scene instanceof Scene_Map)){
  ...
  $gamePlayer._mainMenu = $gamePlayer._mainMenu || {};
  SceneManager._scene._messageWindow = SceneManager._scene._messageWindow || {};
  if (!CTB.Parameters["Left Click Script"] || !(SceneManager._scene instanceof Scene_Map) || $gamePlayer._mainMenu.inMenu || SceneManager._scene._messageWindow._openness > 0 || SceneManager._scene._messageWindow.isAnySubWindowActive()){
    _ti_olbd_ctb.call(this, event);
  }else{
...
TouchInput._onRightButtonDown = function(event) {
    ...
    if (SceneManager._scene instanceof Scene_Map){
      ...
      SceneManager._scene._messageWindow = SceneManager._scene._messageWindow || {};
      if (SceneManager._scene._messageWindow._openness > 0 || SceneManager._scene._messageWindow.isAnySubWindowActive()){
        var x = Graphics.pageToCanvasX(event.pageX);
        var y = Graphics.pageToCanvasY(event.pageY);
        if (Graphics.isInsideCanvas(x, y)) {
            this._onCancel(x, y);
...
// New
TouchInput.updateTurnPlayer = function(event, rect){
  if (ctb.useTurnPlayer){
    var goOn = true;
    if (ctb.turnPlayer_SwitchID) goOn = ($gameSwitches) ? $gameSwitches.value(ctb.turnPlayer_SwitchID) : false;
    if (SceneManager._scene instanceof Scene_Map && SceneManager._scene._messageWindow) if (SceneManager._scene._messageWindow._openness > 0 || SceneManager._scene._messageWindow.isAnySubWindowActive()) goOn = false;
    if (goOn){
      if ($gamePlayer && $gameMap && $dataMap){
        this.dataEx = this.dataEx || {};

        rect = rect || {x:$gamePlayer.screenX(), y:$gamePlayer.screenY(), width:$gameMap.tileWidth()/2, height:$gameMap.tileHeight()/2};

        if (event.pageX < rect.x)             this.dataEx.turnLeft  = true;
        if (event.pageX > rect.x+rect.width)  this.dataEx.turnRight = true;
        if (event.pageY < rect.y-rect.height)             this.dataEx.turnUp    = true;
        if (event.pageY > rect.y+rect.height) this.dataEx.turnDown  = true;

        if (this.dataEx.turnDown){
          $gamePlayer._directionFix = false;
          $gamePlayer.setDirection(2);
          if (eval(ctb.turnFollowers)) $gamePlayer.followers().forEach(function(v){v._directionFix = false;v.setDirection(2);v.refresh();v._directionFix = true;});
          $gamePlayer.refresh();
          this.dataEx.turnDown = false;
          $gamePlayer._directionFix = true;
        }

        if (this.dataEx.turnLeft){
          $gamePlayer._directionFix = false;
          $gamePlayer.setDirection(4);
          if (eval(ctb.turnFollowers)) $gamePlayer.followers().forEach(function(v){v._directionFix = false;v.setDirection(4);v.refresh();v._directionFix = true;});
          $gamePlayer.refresh();
          this.dataEx.turnLeft = false;
          $gamePlayer._directionFix = true;
        }

        if (this.dataEx.turnRight){
          $gamePlayer._directionFix = false;
          $gamePlayer.setDirection(6);
          if (eval(ctb.turnFollowers)) $gamePlayer.followers().forEach(function(v){v._directionFix = false;v.setDirection(6);v.refresh();v._directionFix = true;});
          $gamePlayer.refresh();
          this.dataEx.turnRight = false;
          $gamePlayer._directionFix = true;
        }

        if (this.dataEx.turnUp){
          $gamePlayer._directionFix = false;
          $gamePlayer.setDirection(8);
          if (eval(ctb.turnFollowers)) $gamePlayer.followers().forEach(function(v){v._directionFix = false;v.setDirection(8);v.refresh();v._directionFix = true;});
          $gamePlayer.refresh();
          this.dataEx.turnUp = false;
          $gamePlayer._directionFix = true;
        }
      }
    }
